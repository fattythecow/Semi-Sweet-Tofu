...
public class CJC_Decolorizer : MonoBehaviour {
	...
	void OnTriggerEnter (Collider other)
	{
		...
		if (other.tag == "Player")
		{
			...
			if (damage.IsGreen == false && damage.IsRed == false && damage.IsYellow == false && damage.IsPurple == false)
			{
				sound.GetComponent<AudioSource> ().PlayOneShot (sound.DamageFromEnemySound);
				damage.PlayerHurt = true;
				if (damage.IsSprinting == false)
				{
				...
				else if (damage.IsSprinting == true)
				{
			...
			else if (damage.IsGreen == true | damage.IsRed == true | damage.IsYellow == true | damage.IsPurple == true)
			{
				sound.GetComponent<AudioSource> ().PlayOneShot (decolorsound);
				damage.IsGreen = false;
				damage.IsPurple = false;
				damage.IsRed = false;
				damage.IsYellow = false;
				Health -= 100;
				CJC_Scoring.PlayerScore += 300;
